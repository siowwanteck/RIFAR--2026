# FIaaS MVP Demo Script

Goal: explain the product in 3 minutes.

## Storyline

1. Heavy rain is detected in Taman Sri Muda.
2. IoT sensors report rising water levels in the main drain and attenuation tank.
3. FIaaS predicts flood risk will become critical in 22 hours.
4. The Digital Twin timeline shows how affected areas expand from now to 24 hours.
5. The decision engine recommends activating Pump Station PS2.
6. The operator confirms the recommendation.
7. FIaaS records the decision, starts Pump PS2 in mock mode, lowers water pressure over later ticks, and keeps pump operation human-in-the-loop.

## What To Show

- KPI row: current risk is HIGH and increasing.
- Digital Twin: switch from NOW to +24h.
- Digital Twin: show real OpenStreetMap context around Taman Sri Muda in 2D, then switch to 3D to show the tilted MapLibre digital twin with raised flood zones.
- Forecast panel: show the line chart, risk zones, peak ETA, and confidence.
- Recommended actions: confirm Activate Pump PS2.
- Watch KPIs, forecast, alerts, and map overlays update after confirmation.
- Affected areas: Seksyen 25 and Seksyen 26 appear first.
- System & Data Status: FIaaS is tracking sensor, weather, river, tidal, simulation, prediction, and sync health.

## What Not To Claim

- Do not claim full hydraulic simulation.
- Do not claim automatic SCADA control.
- Do not claim real sensor integration in the MVP.
- Do not claim emergency response is autonomous.
